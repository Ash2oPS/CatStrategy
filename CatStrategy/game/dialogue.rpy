    ...
    menu:   #"menu:" veux dire qu'on va proposer un choix au joueur
        ...
        "Garder ces rats":  #un texte avec :  après c'est l'un des choix pour le joueur, le tete entre guillemets c'est ce que verra le joueur
            if randomChance == 1:    #Une chance sur trois que ça se passe bien
                ...
                $ nourriture += int(membre*0.7)  # le dollar veux dire qu'on écrit en python, c'est surtout utilisé pour modifié ou créer une valeur
                ...
                $ nourriture += int(membre*0.7)  #membre*0.7 c'est la nourriture récup par les chasseurs(en vrai c'est différent et avec du random, mais on va faire simple pour les dialogue)
        ...
        "Payer 15 brins de Catnabis pour purifier les rats" if argent >= 15:  #Troisième choix du joueur qui ne s'affiche que si la condition est vraie, là c'est que si le joueur a 15 argent ou plus
            chat "Génial, payons-nous ces superbes et délicieux rats !"
            $ argent -= 15
            $ nourriture += int(membre*0.7)
...
label fils_bully:
    ...
    menu:
        ...
        "Payer 30 brins de Catnabis les aider à déménager." if argent >= 30:
            $ argent -= 30
            $ boostRecolte+= 0.03
    ...
    menu:
        ...
        "Leur dire de gérer le problème comme il peuvent":
            if randomChance == 1:
                ...
                chat "Nous l'avons réparer avec ce qu'on a pu, mais le silo ne pourra pas contenir autant d'eau qu'avant"
                $ eauMax -= 20
                if eau < 15:
                    $ eau = 0
                else:
